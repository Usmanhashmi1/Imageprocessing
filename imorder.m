...
%defines threshold for matching and finding reference points
Thresh = 5;
confidence = 0.999;
inlierRatio = 0.1;
epsilon = 1.5;
...
%store identitdy matrix of the nImages matrix in T
T = zeros(3, 3, nImgs);
T(:, :, 1) = eye(3);
imgs_feat{nImgs}={};
imgs_dist{nImgs}={};

%finding reference points(SIFT-Features) for stiching, storing in d and f
for i=1: nImgs
...
sequence=[];
sequence(1)=1;
%% forward matching, matching right side of the image to the left of next image
for i=2:nImgs
